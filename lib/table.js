...
Table.prototype.get = function (hashKey, rangeKey, options, callback) {
  ...
  self.sendRequest('get', params, function (err, data) {
    ...
    var item = null;
    if(data.Item) {
      item = self.initItem(self.serializer.deserializeItem(data.Item, self.schema));
    }
...
internals.createItem = function (table, item, options, callback) {
  ...
  internals.callBeforeHooks(self, 'create', start, function (err, data) {
    ...
    // data = result.value;
    if (!_.isEqual(result.value, data)) {
      self.log.warn({ oldValue: data, newValue: result.value }, '[Ignored] Validated value not equal to data passed-in for create');
    }
...
Table.prototype.update = function (item, options, callback) {
  ...
  internals.callBeforeHooks(self, 'update', start, function (err, data) {
    ...
    if (result.error) {
      // return callback(result.error);
      self.log.error({ errors: result.error.details, object: result.error._object }, '[Ignored] ' + result.error.name);
    }

    if (!_.isEqual(result.value, data)) {
      self.log.warn({ oldValue: data, newValue: result.value }, '[Ignored] Validated value not equal to data passed-in for update');
    }
    ...
    self.sendRequest('update', params, function (err, data) {
      ...
      var result = null;
      if(data.Attributes) {
        result = self.initItem(self.serializer.deserializeItem(data.Attributes, self.schema));
      }
...
Table.prototype.destroy = function (hashKey, rangeKey, options, callback) {
  ...
  self.sendRequest('delete', params, function (err, data) {
    ...
    var item = null;
    if(data.Attributes) {
      item = self.initItem(self.serializer.deserializeItem(data.Attributes, self.schema));
    }
  ...
  return function (err, data) {
    ...
    if(data.Items) {
      result.Items = _.map(data.Items, function(i) {
        return table.initItem(table.serializer.deserializeItem(i, table.schema));
      });
